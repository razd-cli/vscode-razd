...
import {
  QuickPickTaskItem,
  QuickPickTaskSeparator
} from './elements/quickPickItem.js';
...
export class TaskExtension {
  ...
  public async update(checkForUpdates?: boolean): Promise<void> {
    // Do version checks
    await taskfileSvc
      .checkInstallation(checkForUpdates)
      .then(
        (status): Promise<PromiseSettledResult<Namespace | undefined>[]> => {
          ...
          vscode.workspace.workspaceFolders?.forEach((folder) => {
            p.push(
              taskfileSvc.read(folder.uri.fsPath, this._nesting, this._status)
            );
          });
  ...
  public registerListeners(context: vscode.ExtensionContext): void {
    ...
    this._watcher.onDidChange(async (uri) => {
      // Ignore temporary taskfiles
      if (!this._isTempFile(uri)) {
        await this._onDidTaskfileChange();
      }
    });
  ...
  private _isTempFile(uri: vscode.Uri): boolean {
    const fileName = uri.fsPath.split(/[\\/]/).pop() || '';
    const isTemp = fileName.startsWith('.') && fileName.includes('.tmp-');
    if (isTemp) {
      log.info(`Ignoring temporary file: ${fileName}`);
    }
    return isTemp;
  }
  ...
  private _onDidChangeConfiguration(event: vscode.ConfigurationChangeEvent) {
    ...
    if (
      ...
      event.affectsConfiguration(oldConfigKey)
    ) {
      settings.update();
      this._nesting = settings.tree.nesting;
      this._status = settings.tree.status;
