...
export class TaskTreeDataProvider implements vscode.TreeDataProvider<TreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<TaskTreeItem | undefined> =
    new vscode.EventEmitter<TaskTreeItem | undefined>();
  readonly onDidChangeTreeData: vscode.Event<TaskTreeItem | undefined> =
  ...
  getChildren(parent?: TreeItem): Thenable<TreeItem[]> {
    ...
    if (
      parent instanceof WorkspaceTreeItem ||
      parent instanceof NamespaceTreeItem
    ) {
      return Promise.resolve(
        this.createTreeItems(
          parent.workspace,
      ...
      for (const task of tasks) {
        treeItems = treeItems.concat(
          new TaskTreeItem(
            ...
            {
              command: 'vscode-razd.goToDefinition',
